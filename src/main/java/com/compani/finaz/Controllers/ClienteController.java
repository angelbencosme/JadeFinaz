...
public class ClienteController extends JdbcStatemant {

    private static ClienteController MANEJO = null;
...
//    public static void save(Cliente cli) {
//        if (cli.getId() != null) {
////            update(cli);
//            return;
//        }
...
//        SQl_EXECUTOR.setJdbcConnection(JdbcManager.get());
//
//        return SQl_EXECUTOR.query(sql, new ClienteRowMapper());
//
//    }
//
...
//
//    private static void update(Cliente cli) {
//        try {
//            String sql = "UPDATE cliente SET identificacion = ?,nombre = ?, apellido = ?, "
//                    + "estado_civil = ? ,fecha_nacimiento =?, sexo =?, telefono =? ,"
//                    + "celular =? ,email = ? ,limite =?, fecha_registro =?, ocupacion =?,"
//                    + "cuidad =?, dirrecion =?,empresa =? ,cargo =? , jefe =? , fecha_ingreso =?,"
//                    + "ingreso =? ,cuidad_trabajo =? , telefono_trabajo =? ,dirrecion_trabajo =? WHERE idcliente = ? ";
//
//            PreparedStatement ps = Conexion.getConexion().prepareStatement(sql);
//
//            ps.setString(1, cli.getIdentificacion());
